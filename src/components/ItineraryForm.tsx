...
const ItineraryForm = ({ 
  ...
  return (
    <form 
      ...
      className="relative overflow-hidden rounded-2xl max-w-xl w-full animate-scale-in bg-white/90 backdrop-blur-lg border border-orange-200 shadow-xl p-6"
    >
      ...
      <div className="space-y-4 relative z-10">
        <div>
          <label htmlFor="destination" className="text-sm font-medium mb-1 block text-orange-800">
            Destination
          </label>
          <div className="relative">
            <Input
              ...
              className="w-full border-orange-200 focus:border-orange-400 focus:ring-orange-400 pl-10"
              value={destination}
              onChange={(e) => {
            ...
            {showSuggestions && filteredDestinations.length > 0 && (
              <div className="absolute z-10 w-full mt-1 bg-white border border-orange-200 rounded-md shadow-lg overflow-hidden">
                {filteredDestinations.map((dest) => (
                  <div 
                    key={dest.name}
                    className="p-2 hover:bg-orange-50 cursor-pointer flex items-center border-b border-orange-100 last:border-0"
                    onClick={() => {
                  ...
                  >
                    <div 
                      className="h-10 w-10 rounded-md bg-cover bg-center mr-2 border border-orange-200"
                      style={{ backgroundImage: `url(${dest.image})` }}
                    />
                    <div>
                      <div className="font-medium text-orange-800">{dest.name}</div>
                      <div className="text-xs text-orange-600">{dest.tags.join(' • ')}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label htmlFor="days" className="text-sm font-medium mb-1 block text-orange-800">
              Number of Days
            </label>
            <Select value={days} onValueChange={setDays}>
              <SelectTrigger id="days" className="w-full border-orange-200">
          ...
          <div>
            <label htmlFor="startDate" className="text-sm font-medium mb-1 block text-orange-800">
              Start Date (Optional)
            </label>
            <div className="relative">
              <Input
